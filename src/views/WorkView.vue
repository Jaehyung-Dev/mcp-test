<template>
  <div class="category-view">
    ...
    <div class="fortune-content">
      ...
      >
        {{ hasGenerated ? '이미 생성되었습니다' : '직장운 확인하기' }}
      </button>
    </div>
  </div>
</template>
...
const tabs = [
  { id: 'today', label: '오늘의 직장운' },
  { id: 'week', label: '이번 주 직장운' },
  { id: 'month', label: '이번 달 직장운' },
  { id: 'year', label: '올해 직장운' }
]
...
const categoryScore = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.work?.score || 0
})

const categoryMessage = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.work?.message || ''
})

const categoryDetails = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.work?.details || ''
})
</script>
