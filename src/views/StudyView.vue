<template>
  <div class="category-view">
    ...
    <div class="fortune-content">
      ...
      >
        {{ hasGenerated ? '이미 생성되었습니다' : '학업운 확인하기' }}
      </button>
    </div>
  </div>
</template>
...
const tabs = [
  { id: 'today', label: '오늘의 학업운' },
  { id: 'week', label: '이번 주 학업운' },
  { id: 'month', label: '이번 달 학업운' },
  { id: 'year', label: '올해 학업운' }
]
...
const categoryScore = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.study?.score || 0
})

const categoryMessage = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.study?.message || ''
})

const categoryDetails = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.study?.details || ''
})
</script>
