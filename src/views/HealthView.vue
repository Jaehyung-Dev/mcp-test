<template>
  <div class="category-view">
    ...
    <div class="fortune-content">
      ...
      >
        {{ hasGenerated ? '이미 생성되었습니다' : '건강운 확인하기' }}
      </button>
    </div>
  </div>
</template>
...
const tabs = [
  { id: 'today', label: '오늘의 건강운' },
  { id: 'week', label: '이번 주 건강운' },
  { id: 'month', label: '이번 달 건강운' },
  { id: 'year', label: '올해 건강운' }
]
...
const categoryScore = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.health?.score || 0
})

const categoryMessage = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.health?.message || ''
})

const categoryDetails = computed(() => {
  return fortuneData.value[currentTab.value]?.categories?.health?.details || ''
})
</script>
